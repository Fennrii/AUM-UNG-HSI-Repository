...
parser.add_argument(
    '-d', '--dataset', dest='dataset', default='IndianPines', help="Name of dataset.")
parser.add_argument(
    '-o',
    '--optimizer',
    dest='optimizer',
...
TRAINING_TIME = []
TESTING_TIME = []
#added for container timing Sept 12 22
TOTAL_TIME = []
...
for index_iter in range(ITER):
    ...
    if not os.path.isdir('/images/models/'):
            os.makedirs('/images/models/')
    torch.save(
        net.state_dict(), "/images/models/S3KAIResNetpatch_" + str(img_rows) + '_' +
    ...
    #added for container timing Sept 12 22
    TOTAL_TIME.append((toc1-tic1)-(toc2-tic2))
    ELEMENT_ACC[index_iter, :] = each_acc

# # Map, Records
print("--------" + " Training Finished-----------")
if not os.path.isdir('/images/report/'):
            os.makedirs('/images/report/')
record.record_output(
    OA, AA, KAPPA, ELEMENT_ACC, TRAINING_TIME, TESTING_TIME,
    '/images/report/' + 'S3KAIResNetpatch:' + str(img_rows) + '_' + Dataset + 'split'
    + str(VALIDATION_SPLIT) + 'lr' + str(lr) + PARAM_OPTIM + '_kernel_' +
    str(PARAM_KERNEL_SIZE) + '.txt')
if not os.path.isdir('/images/classification_maps/'):
            os.makedirs('/images/classification_maps/')
Utils.generate_png(
    all_iter, net, gt_hsi, Dataset, device, total_indices,
    '/images/classification_maps/' + 'S3KAIResNetpatch:' + str(img_rows) + '_' +
